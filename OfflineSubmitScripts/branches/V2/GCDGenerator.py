...
from libs.loger import DummyLogger, get_logger
...
def main(run_id, production_version, snapshot_id, outdir, logger):
    ...
    # Format paths
    gcd_data_path = run.format(config.get('L2', 'GCDDataPath'))
    gcd_all_path = run.format(config.get('L2', 'AllGCDPath'))
    gcd_verified_path = run.format(config.get('L2', 'VerifiedGCDPath'))
    ...
    if os.path.isfile(gcd_data_path) and os.path.getsize(gcd_data_path) > 0:
        ...
        infiles = sorted(run.get_pffilt_files())
...
if __name__ == '__main__':
    ...
    parser.add_argument("--logfile", type = str, default = None, help = "Defines a logfile. If defined, the python log module is used and you can use loglevels. If not defined, everything will be printed.")
    parser.add_argument('--run-id', help = "Run number", dest = "run", type = int)
    ...
    if args.logfile:
        logger = get_logger(args.loglevel, args.logfile)
