...
def main(run_ids, args, config, logger):
    ...
    for run in runs:
        ...
        if not os.path.isfile(sps_gcd_actual):
            logger.error('Did not find finally the GCD file in the tar file. Expected path: {0}'.format(sps_gcd_actual))
            counter.count('error')
            continue
...
if __name__ == '__main__':
    ...
    if args.cron:
        if not config.getboolean('PoleGCDChecks', 'CronEnabled'):
    ...
    if not len(runs):
        ...
        if args.recheck_failed:
            recheck = 'OR gcd_pole_validation > 0'
        else:
            recheck = ''
        ...
        sql = """
            ...
            WHERE (run_id BETWEEN {first_run} AND {last_run} OR run_id IN ({test_runs})) AND
                ...
                (gcd_pole_validation IS NULL {recheck}) AND
