...
class PostgreSqlConnection:
    ...
    def _connect(self):
        self.__connection = psycopg2.connect(user = self._user, password = self._password, host = self._host, dbname = self._database)
    ...
    def fetchall(self, sql, UseDict = True, reconnect = True):
        ...
        if UseDict:
            cursor_type = psycopg2.extras.RealDictCursor

        ret = None

        try:
            with self.__connection.cursor(cursor_factory=cursor_type) as cursor:
                cursor.execute(sql)
                ret = cursor.fetchall()
        except psycopg2.Error as e:
            if e.args[0] == 2006 and reconnect:
