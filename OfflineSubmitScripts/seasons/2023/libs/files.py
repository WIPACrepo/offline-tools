...
class MetaXMLFile(File):
    ...
    def add_post_processing_info(self, script_file, svn_info_from_file = False):
        ...
        if svn_info_from_file:
            svn = SVN(get_rootdir(), self.logger, os.path.join(path.get_tmpdir(), 'svninfo.txt'))
        else:
...
def has_subrun_dstheader_within_good_time_range(subrun, logger):
    ...
    Args:
        subrun (runs.SubRun): The subrun/file
        logger (Logger): The logger
    ...
    def is_frame_in_gtr(frame, run):
        if frame['I3EventHeader'].start_time < run.get_good_start_time():
            return -1
        elif  frame['I3EventHeader'].end_time > run.get_good_stop_time():
            return 1
        else:
            return 0
    ...
    try:
        while f.more():
            ...
            if frame.Has('I3DSTHeader') or frame.Has('I3DST22Header'):
