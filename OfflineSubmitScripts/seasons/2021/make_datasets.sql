insert into datasets (dataset_id, description, season, type, enabled, working_group, comment, iceprod_id) values (@L2, "IC86 2020 Offline L2 Data Production", 2020, "L2", 1, 0, "IceProd2", @L2i ) ;
# update datasets set enabled = 0 where dataset_id = @L2x ;
select * from datasets where dataset_id = @L2x or dataset_id = @L2 ;


insert into datasets (dataset_id, description, season, type, enabled, working_group, comment, iceprod_id) values (@C3, "IC86 2020 L3 Data Production for the Cascade WG", 2020, "L3", 1, 3, "IceProd2", @C3i ) ;
...
insert into level3_config (dataset_id, path, aggregate) values (@C3, '/data/ana/Cscd/IC86-2020/level3/exp/{year}/{month:0>2}{day:0>2}/Run{run_id:0>8}', 1) ;
select * from level3_config where dataset_id = @C3x or dataset_id = @C3 ;


insert into datasets (dataset_id, description, season, type, enabled, working_group, comment, iceprod_id) values (@M3, "IC86 2020 L3 Data Production for the Muon WG", 2020, "L3", 1, 1, "IceProd2", @M3i ) ;
...
insert into datasets (dataset_id, description, season, type, enabled, working_group, comment, iceprod_id) values (@L4, "IC86 2020 L4 Data Production for the Diffuse/Atmospheric Neutrinos WG", 2020, "L4", 1, 5, "IceProd2", @L4i ) ;
...
insert into level3_config (dataset_id, path, aggregate) values (@L4, '/data/ana/Diffuse/IC2010-2014_NuMu/IC86-2012/datasets/finallevel/exp_repro/{year}/{month:02d}{day:02d}/Run{run_id:08d}', 20) ;
