...
def GetParams():
    ...
    parser.add_option("-x", "--dryrun", action="store_true", default=False,
              dest="DRYRUN_", help="don't set status, just print the runs to be affected")

    parser.add_option("-r", "--resubmission", action="store_true", default=False,
              dest="RESUBMISSION_", help="Resubmit the runs. Note that all runs are resubmitted. If a run would be submitted for the very first time, an error is thrown.")
    ...
    params["RESUBMISSION"] = options.RESUBMISSION_
...
def CheckResubmission(StartRun, EndRun, SUBMITLOG):
    SUBMITLOG.write('Check runs for resubmission.')
    print 'Check runs for resubmission.'
    ...
    for Run in Runs:
        if not Run['submitted']:
            SUBMITLOG.write("""Run %s has not been submitted yet."""%(Run['run_id']))
            print """Run %s has not been submitted yet."""%(Run['run_id'])
            Abort = True

    if not Abort:
        print 'Passed resubmission check. All runs are subject to be resubmitted.'
        SUBMITLOG.write('Passed resubmission check. All runs are subject to be resubmitted.')
...
def main(params):
    ...
    if Resubmission:
        if not CheckResubmission(START_RUN, END_RUN, SUBMITLOG):
            print 'At least one run has not been submitted before. Do not use the resubmission flag to submit runs for the first time.'
            print 'Exit'
            SUBMITLOG.write('At least one run has not been submitted before. Do not use the resubmission flog to submit runs for the first time.')
            SUBMITLOG.write('Exit')
            exit(1)
        else:
