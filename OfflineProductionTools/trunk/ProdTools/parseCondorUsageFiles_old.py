...
print "====== Checking for updates at ",CheckTime," ======"

workingDir = '/net/icecube-usr/i3filter'
#workingDir = '/net/icecube-usr/i3filter/chtc'
jAllFiles = glob.glob(workingDir+'/*.json')
jAllFiles.sort(key=lambda x: os.path.getmtime(x))

jFiles = jAllFiles[-7:]
outFile = "/net/icecube-usr/i3filter/NPXCondorUsage_LastWeek.dat"
#if CheckTime.hour >= 1 and CheckTime.hour < 2:
#if True:
if CheckTime.hour in (1,13):
    jFiles = jAllFiles
    outFile = "/net/icecube-usr/i3filter/NPXCondorUsage.dat"


print jFiles

allDataDict = parseFiles(jFiles, "test")
...
#countFiles = 0
#for j in jFiles:
#    #print "processing data from ",j
#    tmpRecords = (json.load(open(j,'r')))
#    date_ = tmpRecords.keys()[0]
#    entries_ = tmpRecords[tmpRecords.keys()[0]]
#    dRecords = entries_[entries_.keys()[0]]

#    #print date_,entries_
#    #print "=====\n"
#    dates_.append(str(date_).replace("-","/") )
...
#for ad in gData:
#    if gNameDataDict.has_key(ad[0]):
#        gNameDataDict[ad[0]] = map(add,[a for a in ad[1:]],[b for b in gNameDataDict[ad[0]]])
#    else:
#        gNameDataDict[ad[0]] = ad[1:]
...
##with open('/net/icecube-usr/i3filter/NPXCondorUsage.dat',"wb") as f:
##    cPickle.dump([dates_,allDataDict,acctDataDict,nameDataDict,cAcctDataList,cNameDataList,gAcctDataList,gNameDataList],f,protocol=cPickle.HIGHEST_PROTOCOL)
