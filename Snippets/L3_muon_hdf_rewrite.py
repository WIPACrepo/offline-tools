...
from icecube import dataio, dataclasses, hdfwriter, icetray

from I3Tray import *
import argparse

parser = argparse.ArgumentParser()
parser.add_argument('-i', help = "infile", type = str, required = True)
parser.add_argument('-o', help = "outfile", type = str, required = True)
...
@icetray.traysegment
def WriteOutput_IC86_2012(tray, name, Suffix, output_hd5):
...
@icetray.traysegment
def WriteOutput_IC86_2015(tray, name, Suffix, output_hd5):
    # Waveform and Pulse Series Names
    raw_inice_readout = "InIceRawData"
    raw_icetop_readout = "IceTopRawData"
    offline_inice_errata = "OfflineInIceCalibrationErrata"
    ...
    for track in ["BestTrack","SplineMPE"]:
        for type in ["HLC","SLC"]:
            level3_keeps+=["{0}Shield{1}".format(track, type)]
            level3_keeps+=["{0}ShieldNHitsOnTime{1}".format(track, type)]
            level3_keeps+=["{0}ShieldNHitsOffTime{1}".format(track, type)]
    ...
    def GetPrimary(frame):
        if frame.Has("I3MCTree") and not frame.Has("MCPrimary1"):
            primaries=frame["I3MCTree"].primaries
            if len(primaries)==1:
                idx=0
            elif "I3MCWeightDict" in frame:
...
tray.Add('I3Reader', FileName = args.i)

tray.AddSegment(WriteOutput_IC86_2015,
    Suffix="HV",
    output_hd5=args.o,
    )
