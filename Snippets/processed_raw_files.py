...
WHERE
    j.dataset_id IN (1911 , 1928)
        AND j.status = 'OK'
        AND u.type = 'INPUT'
        AND u.name NOT LIKE '%GCD.i3.gz'
ORDER BY j.status_changed
""")

fdb = DatabaseConnection.get_connection('filter-db', DummyLogger())

validated = fdb.fetchall('SELECT * FROM i3filter.post_processing WHERE dataset_id IN (1911, 1928) AND validated')
